...
- `eval_ready`: allowed for benchmark evaluation now when `use_for_eval=true`.
...
| Name | Source | Units | Class | Benchmark Use Now | Risk / Notes |
|---|---|---:|---|---|---|
...
| `feedback_cur_bitrate` | Player current ladder rate | bytes_per_second | eval_ready | Yes | Use only on `steady_state` rows with `use_for_eval=true`. |
| `feedback_bwe` | Player/runtime feedback | bytes_per_second | legacy_debug | No | Current estimator is simple last-download fallback; final throughput methodology pending. |
| `feedback_level` | Player current level | quality_level_index | eval_ready | Yes | Use as selected representation level on evaluable rows. |
| `feedback_max_level` | Player ladder metadata | quality_level_index | runtime_only | No | Run context / controller guardrail. |
| `feedback_cur_rate` | Player current ladder rate | bytes_per_second | eval_ready | Yes | Duplicates `feedback_cur_bitrate`; keep for compatibility. |
...
| `feedback_fragment_duration` | parser/player row context | seconds | eval_ready | Yes | Use for segment duration context on evaluable rows. |
...
| `is_init` | Player row assembly | boolean_int | eval_ready | Yes | Init rows must be excluded from benchmark rows. |
...
| `eval_phase` | `core.benchmark_contract` | label | eval_ready | Yes | Canonical phase for benchmark eligibility. |
...
| `use_for_eval` | `core.benchmark_contract` | boolean_int | eval_ready | Yes | Canonical row-level benchmark eligibility flag. |
