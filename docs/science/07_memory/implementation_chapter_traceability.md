...
| Academic baselines | `rate_based` implemented first, `bba` implemented second, and `bola` implemented third as BOLA-basic; MPC and RobustMPC remain deferred. | implementation specs, controller mappings, `tests/test_rate_based_controller.py`, `tests/test_bba_controller.py`, and `tests/test_bola_controller.py`. | Baseline traceability matrix. |
