...
| Academic baselines | `rate_based` implemented first, `bba` implemented second, `bola` implemented third as BOLA-basic, and `mpc` implemented fourth as small-horizon enumerative MPC; RobustMPC remains deferred. | implementation specs, controller mappings, `tests/test_rate_based_controller.py`, `tests/test_bba_controller.py`, `tests/test_bola_controller.py`, and `tests/test_mpc_controller.py`. | Baseline traceability matrix. |
